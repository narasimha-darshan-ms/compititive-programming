...
int main()
{
    ...
    while(!exitCommand)
    {
        ...
        switch(command){
            ...
            case 3 :    cout<<"Enter the element to be removed:\n";
                        cin >> x;
                        bpt.remove(x);
                        break;

            case 4 :    cout<<"Your B+ tree\n";
                        bpt.display(bpt.getRoot());
                        break;

            case 5 :    cout<<"You have entered exit command\n";
                        exitCommand = true;
                        break;
...
void BPTree::insert(int x)
{
    ...
    else
    {
        Node* cursor = root;
        Node* parent;
        while(cursor->IS_LEAF == false)  // similar to search()
        {
            parent = cursor;
            for(int i = 0; i < cursor->size; i++)
            {
        ...
        if(cursor->size < MAX)  //  insert_in_leaf
        {
            int i = 0;
...
{
    if(cursor->size < MAX)      // similar to insert_to-leaf
    {
        int i = 0;
        while(x > cursor->key[i] && i < cursor->size) i++;
